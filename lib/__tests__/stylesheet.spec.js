const { makeCssStyles, makeCssForFnClassName, extractFromClassNames } = require('../stylesheet');
...
describe('makeCssForFnClassName', () => {
  it('should return nul for non-functional class name', () => {
    expect(makeCssForFnClassName('my-class')).toBe(null);
  });

  it('should create styles for functional class name', () => {
    expect(makeCssForFnClassName('Bgc(yellow)')).toBe('.Bgc\\(yellow\\){background-color:yellow}');
  });
});
