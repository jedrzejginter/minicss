...
function ejectBucketToCss(bucket, filename) {
  ...
  VERBOSE && console.log('[minicss] Saving CSS', filename);
...
if (existsSync(CACHE_PATH)) {
  try {
    if (process.env.NODE_ENV !== 'production') {
      ...
      VERBOSE && console.log('[minicss] Read from cache');
    }
      ...
      Program: {
        ...
        exit(path, state) {
          ...
          if (hadAnyMutation || !performedAnyWrite) {
            ejectBucketToCss(bucket, relative(process.cwd(), this.file.opts.filename));
          } else {
            if (VERBOSE) {
              console.log('[minicss] Skipping write, nothing changed ('+
                relative(process.cwd(), this.file.opts.filename)
              +')')
            }
          }
        }
