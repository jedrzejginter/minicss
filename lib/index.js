...
const propMapObject = {
  Ap: ["-moz-appearance", "-webkit-appearance"],
  Ai: "align-items",
  Bd: "border",
...
function extractor(c, stx = "atomic") {
  ...
  const revprops = reverseProp(m[0]);

  return {
    rawClassName: c,
    className: escapeClassName(c),
    state: m[2] ? mapState(m[2]) : 'default',
...
module.exports = function(babel, opts = {}) {
  const { types: t } = babel;
  const { enabled = true, output = 'styles.json' } = opts;
  let queue = [];
    ...
    visitor: {
      Program: {
        ...
        exit(path, state) {
          ...
          if (hadAnyMutation) {
            writeFileSync(output, JSON.stringify(bucket, null, 2), 'utf-8');
            ...
            writeFileSync('out/styles.css', css.join('\n'), 'utf-8');
          } else {
